...
fn release(args: Vec<String>) -> Result<()> {
    ...
    let updated_cargo_lock = if cargo_lock_path.exists() {
        ...
        Some(update_lockfile_version(&cargo_lock, "mdvi", &next_version)?)
    } else {
        None
    };
    ...
    if cargo_lock_path.exists() {
        ...
        run_git(&repo_root, &["add", "Cargo.toml"])?;
    }
...
fn repo_root() -> Result<PathBuf> {
    if let Some(explicit_root) = env::var_os("MDVI_REPO_ROOT") {
...
fn print_release_usage() {
    ...
    eprintln!();
    eprintln!("Examples:");
    eprintln!("  cargo run -p xtask -- release 0.6.4 --dry-run");
